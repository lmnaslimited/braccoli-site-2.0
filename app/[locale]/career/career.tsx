...
export default function Career({ idCareer }: { idCareer: TcareerPageTarget }) {
  ...
  const LaFilteredJobs = StudentJobs.filter((idJob) => {
    ...
    return MatchesSearch && MatchesRoles && MatchesCities;
  });
  ...
  const fnStripMarkdownOrHtml = (iInput: string): string => {
    ...
    const LMarkdownStripped = LHtmlStripped.replace(
      /[*_~`>#\-+=[\]{}()]/g,
      ""
    ).replace(/\[([^\]]+)\]\([^)]+\)/g, "$1"); // [text](link) => text

    // Split by \n, trim each line, and join with commas
    const LCommaSeparated = LMarkdownStripped.split("\n")
      .map((line) => line.trim())
      .filter((line) => line !== "")
      .join(", ");
  ...
  const FilteredTrends = (() => {
    if (SelectedTab === "all") {
      const YoutubeItems = video.filter(
        (v) => v.source.toLowerCase() === "youtube"
      );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <div className="relative mx-auto md:ml-0 w-full space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h4 className="font-semibold text-lg">
                  {idCareer.career.challengeSection.highlight &&
                    idCareer.career.challengeSection.highlight[0]?.label}
                </h4>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                <Link
                  href={
                    idCareer.career?.challengeSection?.buttons?.[0]?.href ?? "#"
                  }
                >
                  <Button size={"lg"}>
                    {idCareer.career?.challengeSection?.buttons?.[0]?.label}{" "}
                    {renderIcon(
                      idCareer.career?.challengeSection?.buttons?.[0]?.icon
                    )}
                  </Button>
                </Link>
              </div>
                    ...
                    <div>
                      <TitleSubtitle
                        idTitle={{
                          ...
                          headingClass:
                            "sm:text-base text-base tracking-normal",
        ...
        <div className="px-4 md:px-24 lg:px-8 mx-auto sm:max-w-xl md:max-w-full lg:max-w-6xl">
          ...
          <Tabs defaultValue="students" className="mb-10">
            ...
            <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 rounded-none">
              {idCareer.career.jobsSection.list
                .slice(0, 2)
                .map((idTitle, iIndex) => (
                  <TabsTrigger
                    value={idTitle.subtitle || `tab-${iIndex}`}
                    onClick={() =>
                      fnSetActiveTab(idTitle.subtitle || `tab-${iIndex}`)
                    }
                    key={iIndex}
            ...
            <TabsContent value="students" className="mt-6">
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-3 mb-4">
                  {Object.entries(FilterOptions).map(([iCategory]) => (
                ...
                <div className="flex flex-col md:flex-row gap-8 items-center">
                  <div className="md:w-2/3 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-4 pt-4">
                      {idCareer.career.jobsSection.buttons
                        .slice(1, 3)
                        .map((idBtn, iIndex) => {
                ...
                <div className="relative overflow-hidden rounded-xl border border-border bg-background p-1">
                  {idCareer.career.planSection.image?.source && (
                    <Image
                      ...
                      alt={
                        idCareer.career.planSection.image.alternate || "image"
                      }
                      width={600}
                      height={600}
                      loading="lazy"
