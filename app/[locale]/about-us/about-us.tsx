...
const renderIcon = (icon: Tbutton['icon'], className?:string) => {
  ...
  return <IconComponent className={className || "h-8 w-8 text-primary/70 mb-8"} />;
};
...
export default function AboutUs({ idAboutUs }: { idAboutUs: TaboutUsPageTarget }){ 
    ...
    return (
        <>
        {/* Hero Section */}
        <section className="py-32 bg-dark text-Secondary">
            <div className="container mx-auto px-6">
            <div className="max-w-5xl mx-auto">
                <TitleSubtitle
                    idTitle={{
                        ...idAboutUs?.aboutUs.heroSection.heading,
                        className: "max-w-4xl text-secondary text-left",
                        headingClass: "text-secondary/70 md:text-sm text-sm tracking-widest uppercase",
                        descripClass: "text-4xl md:text-6xl font-light tracking-tight text-secondary leading-tight",
                    }}
                />
                <div className="h-0.5 w-24 bg-secondary/30 mb-12"></div>
            ...
            <div className="max-w-5xl mx-auto">              
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {idAboutUs?.aboutUs.valuesSection.map((idValues, iIndex)=>(
                        <div className="border border-primary/10 p-10 bg-grayBackground" key={iIndex}>
        ...
        {/* Journey Timeline - Modern, visual */}
        <section className="py-24 bg-grayBackground">
            <div className="container mx-auto px-6">
            <div className="max-w-5xl mx-auto">
          ...
          <div className="max-w-3xl mx-auto text-center">
            <TitleSubtitle
                idTitle={{
                    ...
                    headingClass: "text-3xl md:text-4xl font-light",
                    descripClass: "md:text-xl text-xl text-background/80 max-w-3xl",
                }}
            />
            <div className="grid md:grid-cols-3 gap-6">
                {idAboutUs.aboutUs.ctaSection.buttons.map((idBtn, iIndex) => (
                    idBtn.href ? (
                    <Link href={idBtn.href} className="border border-background/20 p-6 hover:bg-background/5 transition-colors group" key={iIndex}>
                    {renderIcon(idBtn.icon, "h-8 w-8 text-white/70 mb-4 mx-auto")}
                    <p className="font-medium mb-2">{idBtn.label}</p>
                    <ArrowRight className="h-4 w-4 text-background/50 mx-auto group-hover:translate-x-1 transition-transform" />
                  </Link>): (
                    <div
                    key={iIndex}
                    className="border border-background/20 p-6 hover:bg-background/5 transition-colors group"
