...
//   const Industries = await getData<TSlugs[]>("industries")
...
//   const PageSlugs = await getData<Tindustry[]>("industries")

//   const Industry = PageSlugs.find((idIndustry) => idIndustry.slug === slug);
...
export default async function Industries({
  ...
  const PageSlugs = await getData<Tindustry[]>("industries", locale);
  const Industry = PageSlugs.find((idIndustry) => idIndustry.slug === slug);
  console.log("data of locale",Industry)
  if (!Industry) {
    notFound();
  }

  // return <IndustryComp idIndustry={Industry} />;
}

export async function generateStaticParams() {
  const locales = ['en', 'de']; // your supported locales
  const paths: { slug: string; locale: string }[] = [];

  for (const locale of locales) {
    const data = await getData<Tindustry[]>("industries", locale);
    for (const industry of data) {
      paths.push({ slug: industry?.slug , locale });
    }
  }
